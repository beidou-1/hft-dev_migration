...
namespace infra {
class BitgetMarketData : public IExchangeMarketData, public WssHandler {
public:
    BitgetMarketData(net::io_context& ioc, ssl::context& ssl_ctx, const AccountSecret& sec, APIConfig config)
        : IExchangeMarketData(ioc, ssl_ctx, sec, config), client_(ioc_, ssl_ctx_) {}
    ~BitgetMarketData() override = default;
...
private:
    ...
    void on_message_bookticker_sbe(std::string_view msg, uint64_t recv_tsc, uint64_t recv_milli);
