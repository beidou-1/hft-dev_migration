...
namespace infra {
class EdgexMarketData : public IExchangeMarketData, public WssHandler {
public:
    EdgexMarketData(net::io_context& ioc, ssl::context& ssl_ctx, const AccountSecret& sec, APIConfig config)
        : IExchangeMarketData(ioc, ssl_ctx, sec, config), rest_(ioc_, ssl_ctx_) {}
    ~EdgexMarketData() override = default;
