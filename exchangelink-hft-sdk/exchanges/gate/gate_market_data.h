...
namespace infra {
class GateMarketData : public IExchangeMarketData, public WssHandler {
public:
    GateMarketData(net::io_context& ioc, ssl::context& ssl_ctx, const AccountSecret& sec, APIConfig config)
        : IExchangeMarketData(ioc, ssl_ctx, sec, config), client_(ioc_, ssl_ctx_) {}
    ~GateMarketData() override = default;
...
private:
    ...
    using WebSocketClient = WssClient<GateMarketData>;
