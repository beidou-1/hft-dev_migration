...
namespace infra {
class ToobitMarketData : public IExchangeMarketData, public WssHandler {
public:
    ToobitMarketData(net::io_context& ioc, ssl::context& ssl_ctx, const AccountSecret& sec, APIConfig config)
        : IExchangeMarketData(ioc, ssl_ctx, sec, config), rest_(ioc_, ssl_ctx_) {}
    ~ToobitMarketData() override = default;
...
private:
    ...
    void on_message_bookticker(const simdjson::dom::element& doc);
...
private:
    ...
    std::string his_funding_fee_path_{};
